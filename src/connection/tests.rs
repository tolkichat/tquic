...
impl TestPair {
    ...
    fn conn_packets_in(conn: &mut Connection, packets: Vec<(Vec<u8>, PacketInfo)>) -> Result<()> {
...
fn handshake_with_0rtt_data() -> Result<()> {
    ...
    let packet = TestPair::conn_build_packet(&mut test_pair.client, PacketType::ZeroRTT, &[frame])?;
...
fn handshake_with_0rtt_reordered_server_side() -> Result<()> {
    ...
    let frame = TestPair::new_test_stream_frame(content.as_bytes());
    frames.push(frame);
    let packet = TestPair::conn_build_packet(&mut test_pair.client, PacketType::ZeroRTT, &frames)?;
    let info = packets.first().unwrap().1;
...
fn handshake_with_1rtt_reordered_server_side() -> Result<()> {
    ...
    let frame = TestPair::new_test_stream_frame(content.as_bytes());
    frames.push(frame);
    let packet = TestPair::conn_build_packet(&mut test_pair.client, PacketType::OneRTT, &frames)?;
...
fn handshake_with_handshake_reordered_client_side() -> Result<()> {
    ...
    assert_eq!(test_pair.client.is_established(), false);
    let undecryptable_handshake_packets = &test_pair.client.undecryptable_packets.handshake_pkts;
...
fn handshake_multi_cert_with_known_sni() -> Result<()> {
    ...
    for i in 0..conf_selector.len() {
        let mut test_pair =
            TestPair::new_with_server_name(&mut client_config, &mut server_config, &i.to_string())?;
...
fn recv_packet_empty_payload() -> Result<()> {
    ...
    let mut packet = TestPair::conn_build_packet(&mut test_pair.client, PacketType::OneRTT, &[])?;
    let info = TestPair::new_test_packet_info(false);
...
fn recv_packet_stream_frame() -> Result<()> {
    ...
    let frame = TestPair::new_test_stream_frame(content.as_bytes());
    let packet = TestPair::conn_build_packet(&mut test_pair.client, PacketType::OneRTT, &[frame])?;
    let info = TestPair::new_test_packet_info(false);
